...
% Cargamos las imágenes ya recortadas junto con algunos parámetros
% (el factor de escalamiento y los angulos del arreglo) y visualizamos
...
montage(cat(3,Obj, Pln),'Size',[2 4])

%% Mejorar contraste y brillo
Obj1_imadjust = imadjust(Obj(:,:,1));
Obj1_histeq = histeq(Obj(:,:,1));
Obj1_adapthisteq = adapthisteq(Obj(:,:,1));

figure;
montage({Obj(:,:,1),Obj1_imadjust,Obj1_histeq,Obj1_adapthisteq},"Size",[1 4])


[OBJ,PLN] = contraste_brillo(Obj,Pln,opcion_contraste,brillo,gammaa);

% Escalamiento en plano transversal
[xs,ys,Xs,Ys] = escalarT(OBJ(:,:,1),factor_x,factor_y);

% Envolver fase
Psi_obj = envolver(OBJ);
Psi_pln = envolver(PLN);

%% Desenvolvimiento de fase
Phi_obj = cunwrap(Psi_obj);
Phi_pln = cunwrap(Psi_pln);

% Diferencia de fase
delta = Phi_obj - Phi_pln;

% Escalar altura
Z = calcular_z(a,alph,bet,delta,sigma);

% Aplicar máscara y encontrar altura
Z_masked = enmascarar(Xs,Ys,2.97,1.8,Z,1.21);
altura = max(-Z_masked(:));

[~, idx] = max(-Z_masked(:));
[fila_alt, col_alt] = ind2sub(size(Z), idx);
CX = Xs(fila_alt, col_alt);
CY = Ys(fila_alt, col_alt);

disp("Altura: " + num2str(altura) + " cm")

%% Resultados finales
ver_resultados(xs,ys,Psi_obj,Psi_pln,Phi_obj,Phi_pln,delta,-Z,-Z_masked);

%% Cálculo del error

% Esfera teórica
figure;
Zt = sqrt(max(1.5^2 - (Xs - CX).^2 - (Ys - CY).^2, 0));
surf(Xs,Ys,Zt,edgecolor = "none"), colormap gray, axis equal
xlabel('x (cm)'); ylabel('y (cm)'); zlabel('z (cm)')

% Error
figure;
err = abs(-Z_masked-Zt);
surf(xs,ys,err,edgecolor="none"), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)'); zlabel('z (cm)')
disp("Error promedio: " + num2str(mean(err(:))))

% Error en la altura máxima
err_altura = abs(1.5-altura)/1.5*100;
disp("Error en la altura máxima: " + num2str(err_altura) + "%")

%%
function [OBJ, PLN] = contraste_brillo(Obj, Pln, opcion_contraste,brillo,gammaa)
...
for ii = 1:4
    if opcion_contraste == 1
        OBJ(:,:,ii) = imlocalbrighten(imadjust(Obj(:,:,ii),[],[],gammaa),brillo);
        PLN(:,:,ii) = imlocalbrighten(imadjust(Pln(:,:,ii),[],[],gammaa),brillo);
    elseif opcion_contraste == 2
        OBJ(:,:,ii) = imlocalbrighten(histeq(Obj(:,:,ii)),brillo);
        PLN(:,:,ii) = imlocalbrighten(histeq(Pln(:,:,ii)),brillo);
    else
        OBJ(:,:,ii) = imlocalbrighten(adapthisteq(Obj(:,:,ii)),brillo);
        PLN(:,:,ii) = imlocalbrighten(adapthisteq(Pln(:,:,ii)),brillo);
    end
end


figure;
montage({Obj(:,:,1),OBJ(:,:,1)},"Size",[1 2])

end

function [xs,ys,Xs,Ys] = escalarT(Img,factor_x,factor_y)

px_x = size(Img,2); px_y = size(Img,1);
dim_x = px_x*factor_x;
dim_y = px_y*factor_y;
xs=linspace(0,dim_x,px_x);
ys=linspace(0,dim_y,px_y);
[Xs,Ys] = meshgrid(xs,ys);


figure;
imagesc(xs,ys,Img); axis equal; colormap gray

end

function Psi = envolver(P)
...
Psi = atan2(-(P2-P4),(P1-P3));

end

function Z = calcular_z(a,alph,bet,delta,sigma)

z = (delta/(2*pi))*(a/(tan(alph) + tan(bet)));
Z = imgaussfilt3(z,sigma);

end

function Z_masked = enmascarar(X,Y,CX,CY,Z,R)

circ = @(X,Y,R) ((X-CX).^2 + (Y-CY).^2) < R^2;
Mask = circ(X,Y,R);
Z_masked = Mask .* Z;

end

function ver_resultados(xs,ys,P_env,P_env_ref,Q,Q_ref,phi,Z,Z_masked)
...
figure;
subplot(1,2,1)
imagesc(xs,ys,P_env), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)');

subplot(1,2,2)
imagesc(xs,ys,Q), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)');

% Referencia
figure;
subplot(1,2,1)
imagesc(xs,ys,P_env_ref), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)');

subplot(1,2,2)
imagesc(xs,ys,Q_ref), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)');

% Diferencia de fase
figure;
surf(phi,'EdgeColor','none'), colormap gray


% Superficie escalada
figure; 
surf(xs,ys,Z,'EdgeColor','none'), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)'); zlabel('z (cm)')


% Superficie con máscara
figure; 
surf(xs,ys,Z_masked,'EdgeColor','none'), axis equal, colormap gray
xlabel('x (cm)'); ylabel('y (cm)'); zlabel('z (cm)')
